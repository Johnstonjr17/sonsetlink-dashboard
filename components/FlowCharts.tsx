...
function isMultiYear(data: DailyFlowPoint[]): boolean {
...
function CustomFlowTooltip({ active, payload, label, unit, multiYear }: any) {
  ...
  return (
    ...
    >
      <div style={{ fontWeight: 700, marginBottom: 4, color: '#0f172a' }}>{formatDate(label, true)}</div>
...
export function FlowAreaChart({ data, unit }: FlowChartProps) {
  ...
  const multiYear = isMultiYear(data);
  const tickFormatter = (d: string) => formatDate(d, multiYear);
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
        ...
        <Brush dataKey="date" height={28} stroke="#0d9488" fill="#f0fdfa" tickFormatter={tickFormatter} />
      </AreaChart>
    </ResponsiveContainer>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={data} margin={{ top: 24, right: 24, left: 0, bottom: 0 }}>
        ...
        <Brush dataKey="date" height={28} stroke="#6366f1" fill="#eef2ff" tickFormatter={tickFormatter} />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
